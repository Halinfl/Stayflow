...
export default function CampaignCard({ campaign }: CampaignCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group"
    >
      <Link href={href} className="block cursor-pointer" aria-label={`View Stay Shop for ${campaign.name}`}>
      ...
      <div className="px-1 pt-3">
        <div className="flex items-start justify-between gap-2">
          <div>
            <Link href={href}>
              <h3 className="font-bold leading-tight text-brand-charcoal transition-colors hover:text-brand-teal">
                {campaign.name}
              </h3>
            </Link>
            <p className="mt-0.5 flex items-center gap-1 text-sm text-brand-muted">
              <MapPin size={13} /> {campaign.location}
            </p>
          </div>
          <span className="flex shrink-0 items-center gap-1 text-sm font-semibold text-brand-charcoal">
